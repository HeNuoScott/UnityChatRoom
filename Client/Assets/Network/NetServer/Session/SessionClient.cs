...
namespace Network.NetServer
{
    public class SessionClient
    {
        ...
        public void Initialize(Socket socket, int receiveTimeout = 1000)
        {
            ...
            socket.BeginReceive(readBuffer, 0, readBuffer.Length, SocketFlags.None, ReceiveCallBack, new SActionHandler(this));
        }
        ...
        public void Send(DataPackage packet)
        {
            ...
            {
                NetLog.Log(NetLogLevel.Error, e.Message);
            }
        }
        ...
        private void ReceiveCallBack(IAsyncResult ar)
        {
            try
            {
                ...
                if (count <= 0)
                {
                    NetLog.Log(NetLogLevel.Warning, $"收到 {GetRemoteAddress()} 断开连接");
                    Close();
                    return;
                }
                ...
                //数据解析
                SActionHandler actionHandler = ar.AsyncState as SActionHandler;
                DataPackage packet;
